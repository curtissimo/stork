...
definers.composes = function(name, entity) {
  ...
  this.properties[name] = {
    type: ['null', 'array']
  , entity: Array.prototype.slice.call(arguments, 1)
  };
  this.childProperties.push(name);
  this.composedViews[name] = Array.prototype.slice.call(arguments, 1);
};

definers.datetime = function(name, options) {
  var needsCoercion = true
    ;
  if (name === undefined) {
...
definers.object = function(name, options, definer) {
  ...
  if (definer) {
    ...
    this.coercedProperties = this.coercedProperties.concat(subschema.coercedProperties.map(function (p) {
      return name + '.' + p;
    }));
  }
};
...
definers.view = function(name, keys) {
  ...
  if (keys === undefined) {
    throw new Error('view definer requires key definitions or a function');
  }

  if (typeof keys === 'function') {
    this.views[name] = keys;
  } else {
  if (keys.length === undefined || keys.length === 0) {
    throw new Error('view definer requires key definitions in the array');
  }
