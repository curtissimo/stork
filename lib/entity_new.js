...
function validateProto() {
  ...
  coercedProperties.forEach(function(prop) {
    var propParts = prop.split('.')
      , target = o
      , lastProp = propParts[propParts.length - 1]
      , i
      ;

    for (i = 0; target[propParts[i]] !== undefined && target[propParts[i]] !== null && i < propParts.length - 1; i += 1) {
      target = target[propParts[i]];
    }

    if (!target[lastProp] || !target[lastProp].valueOf || prop === 'createdOn') {
      return;
    }
    oldValues[prop] = target[lastProp];
    target[lastProp] = target[lastProp].valueOf();
  });
  ...
  Object.keys(oldValues).forEach(function(prop) {
    ...
    for (i = 0; o[propParts[i]] !== undefined && o[propParts[i]] !== null && i < propParts.length - 1; i += 1) {
      target = target[propParts[i]];
    }
    target[lastProp] = oldValues[prop];
  });
...
module.exports.newForTypeNameAndSchema = function(typeName, schema) {
  return function(id, proto, audit) {
    return entityNew(typeName, schema, id, proto, audit);
  };
};
